...
namespace MoreMountains.Tools
{
	...
	public class AIBrain : MonoBehaviour
	{
        ...
        protected Dictionary<string, object> AIMemories = new Dictionary<string, object>();

        public void SetMemory(string key, object value)
        {
            AIMemories[key] = value;
        }

        public T GetMemory<T>(string key)
        {
            if (AIMemories.TryGetValue(key, out var value) && value is T typedValue)
            {
                return typedValue;
            }
            return default;
